...
void SCManager::setSCdistThres(double _new_thres) { SC_DIST_THRES = _new_thres; }// SCManager::setThres

void SCManager::setMaximumRadius(double _max_r) { PC_MAX_RADIUS = _max_r; }// SCManager::setMaximumRadius
  ...
  Eigen::MatrixXd &_curr_desc)
{
  ...
  polarcontext_tree_batch_->index->findNeighbors(knnsearch_result,
    &curr_key[0] /* query */,
...
std::pair<int, float> SCManager::detectLoopClosureID(void)
{
  ...
  if (tree_making_period_conter % TREE_MAKING_PERIOD_ == 0)// to save computation cost
  {
    ...
    polarcontext_invkeys_to_search_.assign(
      polarcontext_invkeys_mat_.begin(), polarcontext_invkeys_mat_.end() - NUM_EXCLUDE_RECENT);
