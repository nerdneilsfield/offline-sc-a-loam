...
using SCPointType = pcl::PointXYZI;// using xyz only. but a user can exchange the original bin
...
{
public:
  SCManager() = default;// reserving data space (of std::vector) could be considered. but
...
public:
  // hyper parameters ()
  const double LIDAR_HEIGHT = 2.0;// lidar height : add this for simply directly using lidar scan in
  ...
  // tree
  const int NUM_EXCLUDE_RECENT = 30;// simply just keyframe gap (related with loopClosureFrequency in
  ...
  double SC_DIST_THRES = 0.2;// 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy,
                             // DCS) + icp fitness threshold / if not, recommend 0.1-0.15
  // const double SC_DIST_THRES = 0.7; // 0.4-0.6 is good choice for using with
  // robust kernel (e.g., Cauchy, DCS) + icp fitness threshold / if not,
  // recommend 0.1-0.15

  // config
  const int TREE_MAKING_PERIOD_ = 30;// i.e., remaking tree frequency, to avoid non-mandatory every
                                     // remaking, to save time cost / in the LeGO-LOAM integration, it is
                                     // synchronized with the loop detection callback (which is 1Hz) so it
                                     // means the tree is updated evrey 10 sec. But you can use the smaller
                                     // value because it is enough fast ~ 5-50ms wrt N.
